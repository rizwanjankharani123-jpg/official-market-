...
import { Lock, ShieldCheck, Mail, Key, X, Eye, EyeOff, ShieldAlert } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-md bg-[#090d16] border border-cyan-500/40 rounded-2xl sm:rounded-3xl p-5 sm:p-8 shadow-2xl shadow-cyan-950/60 text-left text-slate-200 my-auto max-h-[92vh] overflow-y-auto">
        ...
        <div className="space-y-2 mb-6">
          ...
          <h2 className="text-xl font-bold text-white font-mono">Admin Login</h2>
          <p className="text-xs text-slate-400 leading-relaxed">
            Restricted administrative gateway. Sign in with authorized administrator credentials.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full mt-2 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:brightness-110 text-black font-extrabold text-xs transition-all flex items-center justify-center gap-2 shadow-lg shadow-cyan-500/20 disabled:opacity-50 cursor-pointer"
          >
