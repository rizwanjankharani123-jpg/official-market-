...
export const FeaturedProducts: React.FC<FeaturedProductsProps> = ({
  ...
  const { products, settings, getProductSalesStats, recordFreeDownload, setActiveView, toggleWishlist, isInWishlist } = useApp();
  const [downloadingId, setDownloadingId] = useState<string | null>(null);

  // Filter ONLY published and featured products from real Firestore collection
  const featuredProducts = products
    .filter((p) => p.status === 'published' && Boolean(p.featured))
    .sort((a, b) => {
      // Sensible ordering by createdAt desc if available
  ...
  // If there are no featured products, cleanly hide the section (or return null)
    ...
    <section id="featured-products" className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6 text-left">
        <div className="space-y-3">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/30 text-xs font-mono font-bold shadow-lg shadow-amber-500/5">
            <Sparkles className="w-3.5 h-3.5 text-amber-400 animate-pulse" />
            <span>⭐ FEATURED PRODUCTS</span>
          </div>
          <h2 className="text-2xl sm:text-4xl font-black text-white tracking-tight">
            Curated Highlights & Top Releases
          </h2>
          <p className="text-sm sm:text-base text-slate-400 max-w-2xl">
            Specially handpicked applications and flagship engineering releases by Aftab. Verified architecture with direct access.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-900/90 hover:bg-slate-800 text-cyan-400 hover:text-cyan-300 text-xs font-mono border border-white/10 hover:border-cyan-500/30 transition-all cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {featuredProducts.map((product) => {
          ...
          return (
            ...
            >
              {/* Subtle Featured Glow Header Accent */}
              <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-amber-500 via-cyan-400 to-indigo-500 z-10" />
              ...
              <div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  {/* Dynamic Sales / Downloads / Views Indicator */}
                  {(() => {
